...
exports.createArticle = async (req, res) => {
  ...
  try {
    ...
    // 允许草稿包含 data:image，用于编辑器粘贴暂存；发布/非草稿默认禁止，可用环境变量放开
    const envAllow = String(process.env.ALLOW_DATA_URI_IN_ARTICLE || 'false').toLowerCase() === 'true';
    const isDraft = (status || 'draft') === 'draft';
    const allowDataUri = envAllow || isDraft;
    ...
    if (typeof content === 'string' && isDraft && /data:image\//i.test(content)) {
...
exports.getArticleById = async (req, res, next) => {
  try {
    const { sequelize } = require('../models');
    const { fields, include = 'categories,tags,user' } = req.query;
    ...
    // 原子自增浏览量
    try { await Article.increment('ViewCount', { by: 1, where: { PostID: article.PostID } }); } catch (_) {}

    const data = article.toJSON();
    if (Array.isArray(attributesOption)) {
      // 若未显式请求 Content，保持轻量：不返回 Content
      if (!attributesOption.includes('Content')) delete data.Content;
    }
    if (typeof data.ViewCount === 'number') data.ViewCount += 1;
...
exports.updateArticle = async (req, res) => {
  try {
    ...
    try {
      ...
      // 允许草稿包含 data:image，用于编辑器粘贴暂存；发布/非草稿默认禁止，可用环境变量放开
      const envAllow = String(process.env.ALLOW_DATA_URI_IN_ARTICLE || 'false').toLowerCase() === 'true';
      const isDraft = (status || article.Status || 'draft') === 'draft';
      const allowDataUri = envAllow || isDraft;
      if (!allowDataUri && typeof content === 'string' && /data:image\//i.test(content)) {
        return res.status(400).json({ code: 1, message: '不允许直接提交 data:image；请先通过附件接口上传并替换为 /api/attachments/{id}/preview' });
      }
...
exports.updateArticleStatus = async (req, res) => {
  try {
    ...
    await article.save();
    res.json({ code: 0, message: '状态更新成功', data: article });
  } catch (err) {
    res.status(500).json({ code: 1, message: '服务器错误', error: err.message });
  }
};
...
exports.publishArticle = async (req, res) => {
  try {
    ...
    article.PublishedAt = new Date();
    await article.save();
    res.json({ code: 0, message: '文章发布成功', data: article });
  } catch (err) {
    res.status(500).json({ code: 1, message: '服务器错误', error: err.message });
  }
};
...
exports.archiveArticle = async (req, res) => {
  try {
    ...
    await article.save();
    res.json({ code: 0, message: '文章归档成功', data: article });
  } catch (err) {
    res.status(500).json({ code: 1, message: '服务器错误', error: err.message });
  }
};
...
exports.setFeaturedImage = async (req, res) => {
  try {
    ...
    await article.save();
    res.json({ code: 0, message: '封面图设置成功', data: article });
  } catch (err) {
    res.status(500).json({ code: 1, message: '服务器错误', error: err.message });
  }
};
...
exports.incrementViewCount = async (req, res) => {
  try {
    ...
    await article.save();
    res.json({ code: 0, message: '浏览次数更新成功', data: { viewCount: article.ViewCount } });
  } catch (err) {
    res.status(500).json({ code: 1, message: '服务器错误', error: err.message });
  }
};
