...
async function findPublicTags() {
  const [rows] = await sequelize.query(`
    SELECT DISTINCT
      ...
      t.CreatedAt
    FROM tags t
    INNER JOIN posttags pt ON pt.TagID = t.TagID
    INNER JOIN posts p ON p.PostID = pt.PostID AND p.Status = 'published'
...
exports.getTags = async (req, res, next) => {
  try {
    if (!canReadFullTaxonomy(req.user)) {
      const tags = await findPublicTags();
